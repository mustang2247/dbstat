...
public class DBReaderPlugin extends Plugin
{
  ...
  {
    logger.info("dbreader plugin init begin");
    ConfigHandler conf = (ConfigHandler)event.getContext();
    initDBPool(conf);
    testConnection();
    logger.info("dbreader plugin init end");
    return null;
  }
  ...
  {
    logger.info("dbreader plugin destroy begin");
    if (dataSource != null){
      dataSource.close();
    }
    logger.info("dbreader plugin destroy end");
    return null;
  }
  ...
  {
    logger.info("dbreader plugin service begin");
    event.setContext(dataSource);
    logger.info("dbreader plugin service end");
    return event;
  }

  private static void initDBPool(ConfigHandler conf){
    Map<String,Object> map = conf.getReaderMap();
...
//    p.setRemoveAbandonedTimeout(120);
  ...
  private void testConnection(){
    ...
    try {
      ...
      while (rs.next()) {
        logger.info("dbreader plugin test connection success.");
      }
      rs.close();
      st.close();
    } catch (Exception e) {
      logger.error("dbreader plugin test connection fail. " + e);
      System.exit(1);
    } finally {
